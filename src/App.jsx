    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100 font-sans">
      ...
      <header className="sticky top-0 z-50 border-b-2 border-raceRed bg-black/90 backdrop-blur-md">
        <nav className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 md:px-8">
          <a href="#accueil" className="text-xl font-black tracking-widest text-raceRed uppercase">
              ...
              <li key={id}>
                <a
                  ...
                  className={`px-3 py-1.5 rounded text-sm transition-all duration-200 ${
                    activeNav === id
                      ? 'text-white border-b-2 border-raceRed rounded-none'
          ...
          <div className="md:hidden bg-black border-t border-zinc-800 px-4 pb-4">
            {navItems.map(([id, label]) => (
              <a
                ...
                className="block py-2 text-sm text-zinc-300 hover:text-raceRed transition-colors"
              >
                {label}
              </a>
            ))}
      ...
      <main className="mx-auto max-w-7xl px-4 pb-16 md:px-8">
        ...
        <Section id="accueil" title="">
          ...
          <div className="relative overflow-hidden rounded-3xl border border-zinc-800 bg-gradient-to-br from-zinc-900 via-zinc-950 to-black p-8 md:p-14 mb-10">
            ...
            <div className="absolute bottom-0 right-0 w-64 h-64 bg-[radial-gradient(circle,rgba(225,6,0,0.1),transparent_70%)]" />
            {/* Animated car */}
            <div className="absolute bottom-6 left-0 w-full overflow-hidden h-12 opacity-30">
              <div className="animate-drift flex items-center">
                <div className="h-4 w-32 rounded-full bg-raceRed shadow-lg" />
                <div className="ml-1 h-2 w-8 rounded-full bg-zinc-300" />
              </div>
            </div>
            <div className="relative">
              ...
              <h1 className="mb-5 text-4xl font-black leading-tight md:text-6xl lg:text-7xl">
                La <span className="text-raceRed">Formule 1</span><br />à portée de main
              </h1>
              <p className="mb-8 max-w-xl text-zinc-400 text-base md:text-lg">
                Résultats, pilotes, histoire, live GP et calendrier — tout en un.
              </p>
              <div className="flex flex-wrap gap-3">
                <a href="#live" className="rounded-xl bg-raceRed px-6 py-3 text-sm font-bold text-white transition hover:bg-red-700 uppercase tracking-wider">
                  Suivre le Live GP
                </a>
                <a href="#calendrier" className="rounded-xl border border-zinc-600 px-6 py-3 text-sm font-bold text-zinc-200 transition hover:border-zinc-400 uppercase tracking-wider">
                  Calendrier 2026
                </a>
              </div>
            </div>
        ...
        <Section id="histoire" title="Histoire de la F1">
          <div className="relative border-l-2 border-zinc-800 pl-6 md:pl-10 space-y-6">
            {historyMilestones.map((item) => (
              <div key={item.year} className="relative">
                <span className="absolute -left-[1.45rem] top-5 h-3 w-3 rounded-full bg-raceRed ring-4 ring-zinc-950 md:-left-[2rem]" />
        ...
        <Section id="pilotes" title="Pilotes 2026">
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {currentDrivers.map((driver) => (
              <article key={driver.name} className="group rounded-2xl border border-zinc-800 bg-zinc-900/50 p-4 hover:border-raceRed/50 hover:bg-zinc-900 transition-all duration-200">
                <div className="flex items-center gap-3 mb-3">
                  <div className="flex h-10 w-10 items-center justify-center rounded-full bg-raceRed/10 border border-raceRed/20">
                    <span className="text-xs font-black text-raceRed">#{driver.number}</span>
                  </div>
                  <div>
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody>
                {champions.map((item, i) => (
                  <tr key={item.year} className={`border-t border-zinc-800 transition-colors hover:bg-zinc-900/70 ${i === 0 ? 'bg-raceRed/5' : ''}`}>
        ...
        <Section id="actualites" title="Actualités">
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {news.map((item) => (
              <article key={item.title} className="group rounded-2xl border border-zinc-800 bg-zinc-900/50 p-5 hover:border-raceRed/40 transition-all duration-200 cursor-pointer">
                <div className="mb-3 flex items-center gap-2">
                  <span className="rounded-full bg-raceRed/15 px-3 py-0.5 text-xs font-bold text-raceRed uppercase tracking-wider">
                    {item.category}
                  </span>
                </div>
                <h3 className="mb-2 font-bold text-white leading-snug group-hover:text-raceRed transition-colors">{item.title}</h3>
                <p className="text-sm text-zinc-400 leading-relaxed">{item.snippet}</p>
              </article>
            ))}
          </div>
          <p className="mt-4 text-center text-xs text-zinc-600">Connexion API F1 prévue · News en temps réel à venir</p>
        </Section>
        ...
        <Section id="live" title="Live GP">
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-6">
            <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
              <div>
                <p className="text-sm text-zinc-400 mb-1">Course en cours</p>
              ...
              <div className="flex items-center gap-4">
                {liveOn && (
                  <div className="flex items-center gap-2 rounded-full bg-raceRed/15 px-3 py-1.5">
                    ...
                    <span className="text-xs font-bold text-raceRed uppercase tracking-wider">En direct</span>
                  </div>
                )}
            ...
            <div className="flex gap-3 mb-5">
              <button
                onClick={startLive}
                disabled={liveOn}
                className="rounded-xl bg-raceRed px-5 py-2.5 text-sm font-bold text-white transition hover:bg-red-700 disabled:opacity-40 disabled:cursor-not-allowed uppercase tracking-wider"
              >
              ...
              {liveOn && (
                <button
                  ...
                  className="rounded-xl border border-zinc-600 px-5 py-2.5 text-sm font-bold text-zinc-300 transition hover:border-zinc-400"
                >
                  Pause
                </button>
              )}
            ...
            <div className="space-y-2 max-h-72 overflow-y-auto scrollbar-thin scrollbar-track-transparent scrollbar-thumb-zinc-700">
              {liveIndex === 0 && !liveOn ? (
                <p className="text-center text-sm text-zinc-600 py-8">Appuyez sur "Lancer le live" pour démarrer la simulation</p>
              ) : (
                visibleFeed.map((msg, idx) => (
                  <div
                    ...
                    className={`rounded-xl px-4 py-2.5 text-sm border ${
                      msg.event
                        ? 'border-raceRed/30 bg-raceRed/5 text-white'
                        : 'border-zinc-800 bg-black/30 text-zinc-300'
                    }`}
                  >
                    <span className="text-zinc-600 text-xs mr-2">Tour {msg.lap}</span>
                    {msg.text}
                  </div>
                ))
              )}
        ...
        <Section id="calendrier" title="Calendrier 2026">
          <div className="grid gap-3 lg:grid-cols-2">
            {calendar.map((race, i) => (
              <article key={race.gp} className={`rounded-2xl border p-4 flex items-center gap-4 transition-all ${
                race.status === 'Prochain'
                  ? 'border-raceRed bg-raceRed/5'
                  : race.status === 'Terminé'
                  ? 'border-zinc-800 bg-zinc-900/30 opacity-60'
                  : 'border-zinc-800 bg-zinc-900/50 hover:border-zinc-700'
              }`}>
                <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-zinc-800 text-xs font-black text-zinc-400 flex-shrink-0">
                  R{i + 1}
                </div>
                ...
                <div className="text-right flex-shrink-0">
                  <p className="text-xs text-zinc-400 mb-1">
                    {new Date(race.date).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short' })}
                  </p>
                  <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${
                    race.status === 'Prochain'
                      ? 'bg-raceRed text-white'
                      : race.status === 'Terminé'
      ...
      <footer className="border-t border-zinc-800 py-6 text-center text-xs text-zinc-600">
        F1Hub 2026 · Construit avec React + Tailwind · Données simulées
      </footer>
    </div>
  )
